...
// ! не працює якщо передати ф-цію (закрниває модалку по кліку в любій області)
...
function closeModal(event) {
  ...
  // todo: коли дописую нижній код для реалізації пролистування по кліку, перестає правильно працювати. після кліку по button не працює ESC
  else if (event.target.classList.contains ("lightbox__container-img--btn")) {
    return;
  } else if (event.target.dataset.action ("lightbox-button-press")) {
    return;
  }
...
function mousePress(event) {
  if (event.target.dataset === pressArrows) {
    return;
  }

  if (event.target.getAttribute === pressArrows) {
		if (toggleSlide(0, UrlsArrLength)) return
		modalImg.src = UrlsArr[UrlsArr.indexOf(modalImg.src) - 1];
		return
  }
  
	if (event.target.classList === pressArrows) {
		if (toggleSlide(UrlsArrLength, 0)) return;
		modalImg.src = UrlsArr[UrlsArr.indexOf(modalImg.src) + 1];
		return
	}
